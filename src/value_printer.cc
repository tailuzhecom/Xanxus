...
void ValuePrinter::init_constant(ostream &o, string name, const_value op) {
	int optypeid = op.get_type().get_id();
	...
	// Compute and print the alignment for the constant
	int alignsize = (optypeid == INT1 || optypeid == INT8)? 1
	  : (optypeid == INT32)? 4 : 8;
	o << ", align " << alignsize << "\n";
}
...
void ValuePrinter::call(ostream &o, vector<op_type> arg_types, string fn_name, 
	...
	if (arg_types.size() > 0) {
		o << "(";
		for (unsigned i = 0, e = arg_types.size(); i < e; i++)
...
operand ValuePrinter::call(vector<op_type> arg_types, op_type result_type,
      string fn_name, bool is_global, vector<operand> args, bool isVarArgs) {
	operand result = make_fresh_operand(result_type);
	call(*stream, arg_types, fn_name, is_global, args, result, isVarArgs);
	return result;
}
operand ValuePrinter::call(op_type result_type, string fn_name, bool is_global) { 
	operand result = make_fresh_operand(result_type);
	vector<op_type> arg_types({});
	call(*stream, arg_types, fn_name, is_global, vector<operand>({}), result);
	return result;
}
operand ValuePrinter::call(op_type result_type, string fn_name, bool is_global, 
			   const operand& arg1) {
	operand result = make_fresh_operand(result_type);
	vector<op_type> arg_types({arg1.get_type()});
	vector<operand> args({arg1});
...
operand ValuePrinter::call(op_type result_type, string fn_name, bool is_global, 
	...
	operand result = make_fresh_operand(result_type);
	vector<op_type> arg_types({arg1.get_type(), arg2.get_type()});
	vector<operand> args({arg1, arg2});
	call(*stream, arg_types, fn_name, is_global, args, result);
	return result;
}
